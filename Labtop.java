...
// Создание конструктора для создания экземпляров класса
    public Labtop(String model, float diagonal, String screenResolution, boolean dvdRom, int hardDisk,
...
// создание метода фильтрации ноутбуков по критериям
    public List<Labtop> filter(Set<Labtop> laptop) {
        ...
        List<Labtop> listLaptop = new ArrayList<>();
        for (Labtop tempLaptop : laptop) {
...
// создание метода фильтрации ноутбуков по выборочной критериям
    public Set<Labtop> newFilter(Set<Labtop> laptop) {
        Scanner scan = new Scanner(System.in);
        Set<Labtop> listLaptop = new HashSet<>(laptop);
